...
enum PhantomError: LocalizedError, CustomDebugStringConvertible {
    ...
    var errorDescription: String? {
        ...
        case .responseHasError(let request, let error, let data, let response):
            return "An error occurred during \(request): \(error.localizedDescription.trim())"
        ...
        case .noEffectMiddleware(let middleware):
            return "Middleware \(middleware) had no effect"
        }
    }
    ...
    var debugDescription: String {
        ...
        case .noEffectMiddleware(let middleware):
            return "PhantomError.noEffectMiddleware(middleware: \(String(reflecting: middleware)))"
        }
    }
